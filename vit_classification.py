...
def main():
    """Main function with hyperparameter combinations"""
    import argparse
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="vit_results.json",
        help="Output JSON file path (default: vit_results.json)",
    )
    parser.add_argument(
        "--job_id",
        type=int,
    ...
    for result in existing_results:
        ...
        # Create a hashable representation of hyperparameters
    ...
    for combo in combinations:
        ...
        if combo_tuple not in existing_hyperparams:
            remaining_combinations.append(combo)

    print(f"\nTotal hyperparameter combinations: {len(combinations)}")
    print(f"Already completed: {len(existing_hyperparams)}")
    ...
    if args.total_jobs > 1:
        ...
        send_update_to_discord(
            f"ViT: Job {args.job_id}/{args.total_jobs}: Processing {len(remaining_combinations)} combinations"
        )
    else:
        send_update_to_discord(
            f"ViT: Starting hyperparameter search with {len(remaining_combinations)} combinations"
        )

    all_results = existing_results.copy()

    # Train with each remaining combination
    for i, hyperparams in enumerate(remaining_combinations, 1):
        ...
        print(f"{'=' * 60}")
        send_update_to_discord(
            f"ViT: Combination {i}/{len(remaining_combinations)} starting"
        )
        ...
        try:
            ...
            # Save after each combination to avoid losing progress
            with open(args.output, "w") as f:
                json.dump(all_results, f, indent=2)
        except Exception as e:
            print(f"Error with combination {i}: {e}")
            send_update_to_discord(f"ViT: ERROR in combination {i}: {str(e)}")
            import traceback
    ...
    for i, result in enumerate(all_results, 1):
        ...
        print(
            f"\nCombination {i}: lr={hp['learning_rate']}, bs={hp['batch_size']}, wd={hp['weight_decay']}, dropout={hp.get('dropout_rate', 0.1)}"
        )
    ...
    print(
        f"\nBest combination (F1 Score: {best_result['test_metrics']['f1_score']:.4f}):"
    )
    ...
    send_update_to_discord(
        f"ViT: BEST RESULT - F1: {best_metrics['f1_score']:.4f}, Acc: {best_metrics['accuracy']:.4f}, Recall: {best_metrics['recall']:.4f} | HP: {best_hp}"
    )
    send_update_to_discord("ViT: All training completed!")
